...
import { WriteMode } from "../scene/WriteMode.jsx";
import { uid } from "../store.jsx";

/* ===================== Mailbox ===================== */

export const emptyDraft = { to: "his", subject: "", greeting: "", body: "", signoff: "", theme: "rose", photos: [] };
...
export function MailboxSection({ mail, saveInbox, draft, setDraft }) {
  ...
  const send = async (payload) => {
    if (!payload.subject.trim()) { setError("What's the letter about? Add a short subject."); return; }
    if (!payload.body.trim()) { setError("The page is still blank. Write something first."); return; }
    if (!payload.signoff.trim()) { setError("Sign it off at the end, e.g. from bunny."); return; }
    setBusy(true);
    setError("");
    try {
      const letter = {
        ...
        body: payload.body,
        signoff: payload.signoff.trim(),
        theme: payload.theme || "rose",
  ...
  if (writingTo) {
    return createPortal(
      <WriteMode
        ...
        error={error}
        onSend={send}
        onClose={() => { setWritingTo(null); setError(""); }}
      />,
      document.body
    );
  }
    ...
    <div style={{ maxWidth: 680, margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button className="btn btn-aqua" onClick={() => { setDraft({ ...draft, to: "his" }); setWritingTo("his"); }}>
            {draft.to === "his" && (draft.subject || draft.body || draft.signoff || draft.greeting) ? "continue writing to him" : "write to him"}
          </button>
          <button className="btn btn-pink" onClick={() => { setDraft({ ...draft, to: "hers" }); setWritingTo("hers"); }}>
            {draft.to === "hers" && (draft.subject || draft.body || draft.signoff || draft.greeting) ? "continue writing to her" : "write to her"}
          </button>
        </div>
      </div>
