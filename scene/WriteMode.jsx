...
export function WriteMode({ recipient, draft, setDraft, onSend, onClose, busy, error }) {
  ...
  const confirmPhoto = () => {
    ...
    const photo = {
      ...
      xPct: 4 + (photos.length % 3) * 10,
      yPct: 8 + Math.floor(photos.length / 3) * 24,
      sizePct: 22,
      rotation: 0,
    };
    setPhotos((p) => [...p, photo]);
  ...
  return (
    <div className="pf-write-mode open" style={themeVars}>
      ...
      <div className="pf-letter-wrap" ref={letterWrapRef}>
        ...
        {photos.map((photo) => (
          <PlacedPhoto key={photo.id} photo={photo} letterWrapRef={letterWrapRef} onChange={(patch) => updatePhoto(photo.id, patch)} onDelete={() => deletePhoto(photo.id)} />
        ))}
      </div>
...
function PlacedPhoto({ photo, letterWrapRef, onChange, onDelete }) {
  const elRef = useRef(null);
  const isMobileLayout = () => window.matchMedia("(max-width: 700px)").matches;
  const wrapWidth = () => (letterWrapRef.current ? letterWrapRef.current.getBoundingClientRect().width : 1);
  ...
  const onPointerDownDrag = (e) => {
    ...
    const startX = e.clientX, startY = e.clientY;
    const w = wrapWidth();
    const origXPct = photo.xPct, origYPct = photo.yPct;
    elRef.current.classList.add("dragging");
    let liveXPct = origXPct, liveYPct = origYPct;
    const move = (ev) => {
      liveXPct = origXPct + ((ev.clientX - startX) / w) * 100;
      liveYPct = origYPct + ((ev.clientY - startY) / w) * 100;
      elRef.current.style.left = liveXPct + "%";
      elRef.current.style.top = liveYPct + "%";
    };
    const up = () => {
      elRef.current.classList.remove("dragging");
      onChange({ xPct: liveXPct, yPct: liveYPct });
      window.removeEventListener("pointermove", move);
      window.removeEventListener("pointerup", up);
    };
    window.addEventListener("pointermove", move);
  ...
  const onResizeDown = (e) => {
    ...
    const startX = e.clientX, startY = e.clientY;
    const w = wrapWidth();
    const origSizePct = photo.sizePct, origXPct = photo.xPct, origYPct = photo.yPct;
    let liveSizePct = origSizePct, liveXPct = origXPct, liveYPct = origYPct;
    const move = (ev) => {
      const deltaPx = ((ev.clientX - startX) + (ev.clientY - startY)) / 2;
      const deltaPct = (deltaPx / w) * 100;
      liveSizePct = Math.max(6, Math.min(85, origSizePct + deltaPct));
      const growPct = liveSizePct - origSizePct;
      liveXPct = origXPct - growPct / 2;
      liveYPct = origYPct - growPct / 2;
      elRef.current.style.width = liveSizePct + "%";
      elRef.current.style.left = liveXPct + "%";
      elRef.current.style.top = liveYPct + "%";
    };
    const up = () => {
      onChange({ sizePct: liveSizePct, xPct: liveXPct, yPct: liveYPct });
      window.removeEventListener("pointermove", move);
      window.removeEventListener("pointerup", up);
    };
    window.addEventListener("pointermove", move);
  ...
  const onRotateDown = (e) => {
    ...
    let liveRotation = photo.rotation;
    const move = (ev) => {
      const rect = elRef.current.getBoundingClientRect();
      const cx = rect.left + rect.width / 2, cy = rect.top + rect.height / 2;
      const angleRad = Math.atan2(ev.clientY - cy, ev.clientX - cx);
      liveRotation = (angleRad * 180) / Math.PI + 90;
      elRef.current.style.transform = `rotate(${liveRotation}deg)`;
    };
    const up = () => {
      handle.classList.remove("dragging");
      onChange({ rotation: liveRotation });
      window.removeEventListener("pointermove", move);
      window.removeEventListener("pointerup", up);
    };
    window.addEventListener("pointermove", move);
  ...
  return (
    <div
      ...
      style={{
        left: photo.xPct + "%", top: photo.yPct + "%", width: photo.sizePct + "%",
...
function PfStyles() {
  return (
    <style>{`
      ...
      .pf-placed-photo { position: absolute; z-index: 2; pointer-events: auto; cursor: grab; touch-action: none; }
      ...
      @media (max-width: 700px) {
        .pf-write-mode .pf-placed-photo { position: static !important; transform: none !important; cursor: default; margin: 14px auto 0; display: block; }
        .pf-write-mode .pf-placed-photo .pf-resize-handle, .pf-write-mode .pf-placed-photo .pf-rotate-handle, .pf-write-mode .pf-placed-photo .pf-rotate-stem { display: none; }
      }
    `}</style>
  );
}
